...
Data.sc = Tbl1 %>% filter(Plant %in% c("322U", "3475U", "354U", "34N42U", "322", "3475", "354", "34N42"))
...
p5 = ggplot(tmp, aes(x = R_Year, y = HIN, shape = Fert))+
    ...
    theme_pub() +
    xlab("Year of release")+
    ylab("N harvest index\n(kg grain N/kg plant N)") +
...
grid.arrange(RightRow, Fertlegend2, ncol = 2, widths = c(1,.1))
```
...
# sub = tmp %>% mutate(FertilizerN = as.numeric(as.character(Fert)),
#            RE_15N = Total_kgFertN_ha/FertilizerN,
#            Fert = factor(Fert)) %>%
#           filter(Fert != "0")
head(sub)
pComp = ggplot(tmp, aes(x = FertN_difference, y = Total_kgFertN_ha, shape = Fert)) +
  geom_point(aes(fill = Fert)) +
  geom_smooth(aes(linetype = Fert), method = "lm", se = FALSE, color = "black") +
   scale_shape_manual(values = c(21,22))+
    theme_pub() +
...
p1 = ggplot(subset(tmp, Fert == "85"), aes(x = R_Year.f, y = T2_N, fill = SeedTreatment)) +
  ...
  theme(axis.text.x = element_text(angle= 60),  axis.title.x = element_blank())+
...
* Additionally there is wide variation in plant growth and N uptake, likely related to spatial variability in field drainage.  Here I check whether early season biomass accumulation can be used as a covariate to improve estimates of release year and fertilizer by adding dry weight at V6 to a model predicting N uptake

```{r}
m3 = lmer(Final_kgN_ha ~ T1_kgdw_ha + R_Year.c*Fert + (1|Rep) + (1|Genotype) + (1|Rep:Row:Pos),  data = Data, na.action = na.exclude)
  anova(m3, ddf = "Kenward-Roger")

```

### Plot ls means to visualize difference
*note that we need to use model with R_Year as a factor in order to calculate lsmeans for each genotype
```{r}

m3 = lmer(Final_kgN_ha ~ T1_kgdw_ha + as.factor(R_Year.c)*Fert + (1|Rep)  + (1|Rep:Row:Pos),  data = Data, na.action = na.exclude)
  anova(m3, ddf = "Kenward-Roger")


  Yield.cld = lsmeans(m3, pairwise~R_Year.c|Fert) %>%
        cld(Letters = c(letters))  %>% 
        as.data.frame %>% 
         filter(!is.na(lsmean)) 
 
 p =   ggplot(Yield.cld, aes(x = R_Year.c, y = lsmean, color = Fert)) +
    geom_point() +
  theme_pub()+ 
...
```
* Including biomass at T1 in model does not improve lsmean estimation nor Fstatistic of R_Year.c


* Here I am trying to get a handle on spatial variation and how the ERA hybrids respond, plotting biomass at V6 vs Final biomass, split by fertilization level.  Here the shapes are hybrids of different ERAs 1 = pre-fertilizer (1930s -50), 2 = increasing fertilizer use (50s-80s) and 3 = high but stable use (90s - present day).  For a given level of early season growth, the modern hybrids show greater biomass accumulation by the end of the season.  This may be an indication they they were more stress tolerant during the saturated early season.  This would also explain why there was greater N uptake from soil.  Since N uptake will be dependent on plant size, differences in plant growth following prolonged saturated soils could lead to differences in N uptake.   


```{r, echo = FALSE}

p2 = ggplot(Data, aes(x = T1_kgdw_ha, y = Stover_kgdw_ha + Grain_kgdw_ha, shape = ERA))+
    
    geom_point(color = "black", size = 2) +
    scale_shape_manual(values = c(0,19,2))+
    theme_pub() +
    xlab("Biomass at V6 (kg/ha)")+
    ylab("Final biomass") +
    #scale_color_manual(values = FertColors) +
    geom_smooth(aes(linetype = ERA), method = "lm", se = FALSE, color = "black") +
    scale_linetype_manual(values = c(3,1,5))+
    facet_wrap(~Fert)
p2
...
Here I am trying to understand the relationship between biomass and N uptake.  There is a very strong relationship.  It also appears that the modern hybrids are shifted right on the x-axis in high fertility plots indicating greater biomass accumulation per unit N uptake:  

```{r, echo = FALSE}

p2 = ggplot(Data, aes(x = Stover_kgdw_ha + Grain_kgdw_ha, y = Final_kgN_ha, shape = ERA))+
